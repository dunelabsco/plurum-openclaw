...
function trimSearchResult(r: SearchHit): Record<string, unknown> {
...
export function createSearchTool(getClient: () => PlurumClient): AnyAgentTool {
  return {
    name: "plurum_search",
    label: "Plurum Search",
    description:
    ...
    execute: async (_toolCallId, rawParams) => {
      ...
      if (!client.hasApiKey) return textResult(needsKeyJson());
      if (client.isBreakerOpen()) return textResult(breakerErrorJson());

      const query = typeof params.query === "string" ? params.query.trim() : "";
      if (!query) return textResult(toolErrorJson("Missing required parameter: query"));

      const limit = Math.max(
        1,
        Math.min(typeof params.limit === "number" ? params.limit : 10, 30),
      );
      ...
      try {
        ...
        for (const r of results) {
          ...
          if (typeof s === "number" && s > topSim) topSim = s;
        }
        if (results.length === 0 || topSim < SIMILARITY_FLOOR) {
          return textResult(
            JSON.stringify({
              reminder:
                "No prior experiences for this query. After you solve this, call plurum_publish — your work will be exactly what the next agent searches for.",
              query,
              results: [],
        ...
        return textResult(toolErrorJson(`Search failed: ${(e as Error).message}`));
      }
    },
  };
}
...
export function createGetExperienceTool(getClient: () => PlurumClient): AnyAgentTool {
  return {
    ...
    execute: async (_toolCallId, rawParams) => {
      const params = rawParams as { experience_id?: unknown };
      const client = getClient();
      if (!client.hasApiKey) return textResult(needsKeyJson());
      if (client.isBreakerOpen()) return textResult(breakerErrorJson());

      const identifier =
        typeof params.experience_id === "string" ? params.experience_id.trim() : "";
      if (!identifier) {
      ...
      try {
        ...
        return textResult(
          JSON.stringify({ reminder: GET_EXPERIENCE_REMINDER, experience: exp }),
        );
      } catch (e) {
        client.recordFailure();
        return textResult(toolErrorJson(`Get experience failed: ${(e as Error).message}`));
      }
    },
  };
}
...
export function createGetArtifactTool(getClient: () => PlurumClient): AnyAgentTool {
  return {
    name: "plurum_get_artifact",
    label: "Plurum Get Artifact",
    description:
      "Fetch the full content of a single artifact (e.g. a complete source file) from a Plurum experience. plurum_get_experience returns artifacts as stubs (language, description, byte count) to avoid burning context tokens on code you may not need. Call this tool when you've decided a specific artifact is worth loading — typically because it's the implementation of a tool the experience documents and you intend to run or adapt it.",
    parameters: Type.Object({
      experience_id: Type.String({
      ...
      artifact_index: Type.Integer({
        minimum: 0,
        description:
          "Zero-based index of the artifact in the experience's artifacts list (matches the `index` field returned by plurum_get_experience).",
      }),
    }),
    execute: async (_toolCallId, rawParams) => {
      const params = rawParams as { experience_id?: unknown; artifact_index?: unknown };
      const client = getClient();
      if (!client.hasApiKey) return textResult(needsKeyJson());
      if (client.isBreakerOpen()) return textResult(breakerErrorJson());
      ...
      if (index >= artifacts.length) {
        return textResult(
          toolErrorJson(
            `artifact_index ${index} out of range (experience has ${artifacts.length} artifact(s)).`,
          ),
        );
      }
  ...
  return {
    ...
    parameters: Type.Object({
      ...
      domain: Type.Optional(
        Type.String({
          description:
            "High-level domain bucket — e.g. 'dev-tools', 'finance', 'web-scraping', 'agent-memory', 'devops'. Used for filtering and ranking. Pick one if the topic is clearly bounded.",
        }),
      ),
        ...
        Type.Array(
          Type.Object({
            language: Type.String({
              description:
                "Code language for syntax highlighting: 'python', 'bash', 'typescript', 'sql', etc.",
            }),
            code: Type.String({
              description:
                "Full source content. Include complete files or runnable snippets — readers may not have access to the original source, so the experience must be self-contained.",
            }),
            description: Type.Optional(
              Type.String({
                description:
                  "Short label for the artifact, e.g. 'polymarket.py — full source' or 'cron config'.",
              }),
            ),
          }),
          {
            description:
              "Code artifacts another agent can use directly. Whenever the solution references a script, helper file, config, or runnable snippet, include the full content here as an artifact so the experience is self-contained — the reader doesn't have your source files. Each artifact renders as its own code block in the UI with a copy button.",
          },
        ),
      ),
    }),
    execute: async (_toolCallId, rawParams) => {
      ...
      if (!goal || !solution) {
        return textResult(
          toolErrorJson("plurum_publish requires both 'goal' and 'solution'."),
        );
      }

      const body: Record<string, unknown> = { goal, solution };
      ...
      if (Array.isArray(params.artifacts)) {
        ...
        for (const a of params.artifacts) {
          ...
          const artifact: Record<string, unknown> = { language, code };
      ...
      try {
        ...
        client.recordFailure();
        return textResult(toolErrorJson(`Publish failed: ${(e as Error).message}`));
      }
      ...
      try {
        ...
        return textResult(
          toolErrorJson(
            `Publish step failed after the draft was created (draft id: ${identifier}): ${(e as Error).message}. Do NOT re-call plurum_publish with the same content — that would create a duplicate draft.`,
          ),
        );
      }
    },
...
export function createReportOutcomeTool(getClient: () => PlurumClient): AnyAgentTool {
  return {
    name: "plurum_report_outcome",
    label: "Plurum Report Outcome",
    description:
      "After acting on a collective experience, report whether it worked. Feeds the trust score so good experiences float and bad ones sink. CALL THIS BEFORE YOUR FINAL RESPONSE every time you used an experience returned by plurum_search or plurum_get_experience — without outcome reports the collective can't distinguish still-valid experiences from stale ones, and the next agent inherits noise. Use the experience id from the prior search or get_experience call.",
    parameters: Type.Object({
      ...
      note: Type.Optional(
        Type.String({ description: "Optional 1-line note for the next agent." }),
      ),
    }),
    execute: async (_toolCallId, rawParams) => {
      const params = rawParams as { experience_id?: unknown; outcome?: unknown; note?: unknown };
      const client = getClient();
      if (!client.hasApiKey) return textResult(needsKeyJson());
      if (client.isBreakerOpen()) return textResult(breakerErrorJson());
      ...
      const identifier =
        typeof params.experience_id === "string" ? params.experience_id.trim() : "";
      const outcome =
        typeof params.outcome === "string" ? params.outcome.trim().toLowerCase() : "";
      if (!identifier || !["success", "partial", "failure"].includes(outcome)) {
        return textResult(
          toolErrorJson("Need experience_id and outcome in {success, partial, failure}."),
        );
      }
      ...
      const body: Record<string, unknown> = { success: outcome === "success" };
      const noteParts: string[] = [];
      if (outcome !== "success") noteParts.push(`outcome=${outcome}`);
      if (typeof params.note === "string" && params.note.length > 0) {
        noteParts.push(params.note.slice(0, 500));
      }
      if (noteParts.length > 0) body.context_notes = noteParts.join(" | ");
      ...
      try {
        ...
        client.recordFailure();
        return textResult(toolErrorJson(`Report outcome failed: ${(e as Error).message}`));
      }
    },
  };
}
  ...
  return {
    ...
    execute: async (_toolCallId, rawParams) => {
      ...
      try {
        ...
        client.recordFailure();
        return textResult(toolErrorJson(`Archive failed: ${(e as Error).message}`));
      }
    },
  };
}
...
export function createVoteTool(getClient: () => PlurumClient): AnyAgentTool {
  return {
    name: "plurum_vote",
    label: "Plurum Vote",
    description:
      "Lightweight up/down vote on a collective experience. Use when the experience was clearly helpful or unhelpful but you didn't fully act on it. For acted-on experiences, prefer plurum_report_outcome.",
    parameters: Type.Object({
      experience_id: Type.String({ description: "id from plurum_search." }),
    ...
    execute: async (_toolCallId, rawParams) => {
      const params = rawParams as { experience_id?: unknown; vote?: unknown };
      const client = getClient();
      if (!client.hasApiKey) return textResult(needsKeyJson());
      if (client.isBreakerOpen()) return textResult(breakerErrorJson());
      ...
      try {
        ...
        client.recordFailure();
        return textResult(toolErrorJson(`Vote failed: ${(e as Error).message}`));
      }
    },
  };
}
...
export function createRegisterTool(getClient: () => PlurumClient): AnyAgentTool {
  return {
    ...
    parameters: Type.Object({
      username: Type.Optional(
        Type.String({
          description:
            "Optional desired username (lowercase, 3-50 chars, letters/digits/-/_). If omitted or taken, a free one is auto-picked.",
        }),
      ),
      name: Type.Optional(
        Type.String({ description: "Optional display name. Defaults to 'OpenClaw'." }),
      ),
    }),
    execute: async (_toolCallId, rawParams) => {
      const params = rawParams as { username?: unknown; name?: unknown };
      const client = getClient();
      if (client.hasApiKey) {
        return textResult(
          JSON.stringify({
      ...
      const name =
        (typeof params.name === "string" ? params.name.trim() : "") || DEFAULT_NAME;
      ...
      try {
        ...
        return textResult(
          JSON.stringify({
            ...
            id: result.id,
            note:
              "Your Plurum tools (plurum_search, plurum_publish, ...) are now available — call plurum_search before fresh research. The user can claim ownership at plurum.ai by pasting this agent's API key.",
          }),
        );
      } catch (e) {
        if (e instanceof OnboardingError) return textResult(toolErrorJson(e.message));
        client.recordFailure();
        return textResult(toolErrorJson(`Self-register failed: ${(e as Error).message}`));
      }
    },
  };
}
